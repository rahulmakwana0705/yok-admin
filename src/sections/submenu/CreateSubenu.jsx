...
const Submenu = () => {
    ...
    const handleColumnAdd = (menuIndex) => {
        ...
        newMenuData[menuIndex].columns.push({ id: newMenuData[menuIndex].columns.length + 1, columnItems: [] });
    ...
    const handleColumnItemItemsAdd = (menuIndex, columnIndex, columnItemIndex) => {
        ...
        newMenuData[menuIndex].columns[columnIndex].columnItems[columnItemIndex].columnItemItems.push({
            id: newMenuData[menuIndex].columns[columnIndex].columnItems[columnItemIndex].columnItemItems.length + 1,
            path: '',
            label: '',
        });
        setMenuData(newMenuData);
    ...
    const handleColumnItemItemInputChange = (menuIndex, columnIndex, columnItemIndex, columnItemItemIndex, field, value) => {
        const newMenuData = [...menuData];
        newMenuData[menuIndex].columns[columnIndex].columnItems[columnItemIndex].columnItemItems[columnItemItemIndex][field] = value;
        setMenuData(newMenuData);
    };
                ...
                {menuData.map((menu, menuIndex) => (
                    <div key={menu.id}>
                        <Grid container spacing={2} sx={{ marginBottom: 2 }}>
                                ...
                                <Grid item xs={12} key={column.id} sx={{ marginBottom: 2 }}>
                                    ...
                                    {column.columnItems.map((columnItem, columnItemIndex) => (
                                        <div key={columnItem.id}>
                                            <Grid container spacing={1} sx={{ marginBottom: 1 }}>
                                                <Grid item xs={6}>
                                                    <TextField
                                                        ...
                                                        onChange={(e) => handleColumnItemInputChange(menuIndex, columnIndex, columnItemIndex, 'path', e.target.value)}
                                                    />
                                                </Grid>
                                                <Grid item xs={6}>
                                                    <TextField
                                                        ...
                                                        onChange={(e) => handleColumnItemInputChange(menuIndex, columnIndex, columnItemIndex, 'label', e.target.value)}
                                                    />
                                                </Grid>
                                            </Grid>
                                            {columnItem.columnItemItems.map((columnItemItem, columnItemItemIndex) => (
                                                <Grid container spacing={1} key={columnItemItem.id} sx={{ marginBottom: 1 }}>
                                                    <Grid item xs={6}>
                                                        <TextField
                                                            label="Path"
                                                            fullWidth
                                                            value={columnItemItem.path}
                                                            onChange={(e) => handleColumnItemItemInputChange(menuIndex, columnIndex, columnItemIndex, columnItemItemIndex, 'path', e.target.value)}
                                                        />
                                                    </Grid>
                                                    <Grid item xs={6}>
                                                        <TextField
                                                            ...
                                                            onChange={(e) => handleColumnItemItemInputChange(menuIndex, columnIndex, columnItemIndex, columnItemItemIndex, 'label', e.target.value)}
                                                        />
                                                    </Grid>
                                                </Grid>
                                            ))}
                                            <Button
                                                variant="contained"
                                                onClick={() => handleColumnItemItemsAdd(menuIndex, columnIndex, columnItemIndex)}
                            ...
                            <Button
                                variant="contained"
                                onClick={() => handleColumnAdd(menuIndex)}
                            >
                                Add Column
                            </Button>
                            <Button
